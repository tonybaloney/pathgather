...
class MockClient(BaseMockClass):
    ...
    def _v1_paths_57a89e01_59a9_4c39_8b64_4eabd512e9f5_comments(self, request, method):
        if method == "GET":
            return self.adapter.response_from_fixture(request, 'tests/fixtures/v1/path_57a89e01-59a9-4c39-8b64-4eabd512e9f5_comments')
        elif method == "POST":
            return self.adapter.response_from_fixture(request, 'tests/fixtures/v1/path_comment_create')
        else:
...
def test_create_path_comment():
    with mock_session_with_class(client.session, MockClient, TEST_URL):
        response = client.paths.create_comment(TEST_PATH_ID, "<p>hello from robot</p>", TEST_USER_ID)
