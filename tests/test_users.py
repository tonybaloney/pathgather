...
class MockClient(BaseMockClass):
    ...
    def _v1_users_4cea0449_666a_433f_8396_857e269449a9_user_skills(self, request, method):
        if method == 'GET':
            return self.adapter.response_from_fixture(request, 'tests/fixtures/v1/users_skills')
        if method == 'POST':
            return self.adapter.response_from_fixture(request, 'tests/fixtures/v1/users_skill')

    def _v1_users_4cea0449_666a_433f_8396_857e269449a9_user_skills_f5bf30a8_412c_4cea_9a64_1d4aae5a40f9(self, request, method):
        if method == 'DELETE':
...
def test_add_skill_by_id():
    with mock_session_with_class(client.session, MockClient, TEST_URL):
        response = client.users.add_skill_by_id(TEST_USER_ID, TEST_SKILL.id, SkillLevel.ALL)
        assert response.level == SkillLevel.EXPERT


def test_add_skill_by_name():
    with mock_session_with_class(client.session, MockClient, TEST_URL):
        response = client.users.add_skill_by_name(TEST_USER_ID, TEST_SKILL.name, SkillLevel.ALL)
        assert response.level == SkillLevel.EXPERT


def test_update_skill():
    with mock_session_with_class(client.session, MockClient, TEST_URL):
        response = client.users.update_skill_level(TEST_USER_ID, TEST_SKILL, SkillLevel.INTERMEDIATE)
        assert response.level == SkillLevel.EXPERT


def test_delete_skill():
    with mock_session_with_class(client.session, MockClient, TEST_URL):
        response = client.users.delete_skill(TEST_USER_ID, "f5bf30a8-412c-4cea-9a64-1d4aae5a40f9")
        assert response is None

        response = client.users.delete_skill(TEST_USER_ID, UserSkill(
            id=TEST_USER_SKILL_ID,
