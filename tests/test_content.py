...
class MockClient(BaseMockClass):
    ...
    def _v1_content_89a07305_0122_4da1_8b40_b99f4a968f88_comments(self, request, method):
        if method == "GET":
            return self.adapter.response_from_fixture(request, 'tests/fixtures/v1/content_89a07305-0122-4da1-8b40-b99f4a968f88_comments')
        elif method == "POST":
            return self.adapter.response_from_fixture(request, 'tests/fixtures/v1/content_89a07305-0122-4da1-8b40-b99f4a968f88_comments_create')
...
def test_create_content_comment():
    with mock_session_with_class(client.session, MockClient, TEST_URL):
        response = client.content.create_comment(TEST_CONTENT_ID, "hello from bot", TEST_USER_ID)
        assert response.message == "hello from bot"
        assert response.user.first_name == "DDU"
        assert response.content.name == "No serial number - how we log it in ITSM / BranchTrack"